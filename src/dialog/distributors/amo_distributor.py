...
async def proceed_new_lead(lead_id, new_status_id=None):
    ...
    if not phone_number:
        ...
        try:
            ...
            utm_source = None
        if utm_source and utm_source == 'Flocktory':
            ...
            if baby_age_month > 42 and segment != "C" and for_online:
                try:
                    ...
                    if chat_id:
                        ...
                        # После неудачной попытки создания чата в Radist.Online добавляем тайм-аут и пытаемся ещё раз
                        logger.info(f"Ошибка создания чата в сделке #{lead_id}, проблема Radist.Online, тайм-аут: 300")
                        await asyncio.sleep(300)
                        chat_id = await RadistOnlineChats.create_new_chat(
                            name=name,
                            phone=phone_number
                ...
                await LeadFetcher.change_lead_status(lead_id=lead_id, status_name='ТРЕБУЕТСЯ МЕНЕДЖЕР')
                await CustomFieldsFetcher.change_status(lead_id=lead_id, text="Требуется менеджер")
                await TaskFetcher.set_task(lead_id=lead_id, task_text=TaskTexts.NEED_MANAGER_TEXT)
    ...
    try:
        ...
        try:
            ...
            if pipeline_id == settings.LOGOPOTAM_PIPELINE_ID:
                ...
                if new_status_name == "новая заявка":
                    ...
                    logger.info(f"Новая сделка #{lead_id} появилась не в статусе новая заявка, пропускаем ее")
            else:
                logger.info(f"Новая сделка #{lead_id} не относится к воронке Логопотам")
        except KeyError:

            # Вебхук на изменение статуса сделки
            try:
                ...
                if pipeline_id == settings.LOGOPOTAM_PIPELINE_ID:
                    ...
                    if lead_exist:
                        ...
                        if new_status_name == "СТАРТ НЕЙРО":
                            logger.info(f"Новая сделка: {lead_id}, {pipeline_id}, {new_status_id}, {new_status_name}")
                            await proceed_new_lead(lead_id, new_status_id)
                else:
                    logger.info(f"Новая сделка #{lead_id} не относится к воронке Логопотам")
            except KeyError:

                # Вебхук на Неразобранное
                try:
                    lead_id = data['unsorted[add][0][lead_id]']
                    pipeline_id = int(data['unsorted[add][0][pipeline_id]'])
                    if pipeline_id == settings.LOGOPOTAM_PIPELINE_ID:
                        await LeadFetcher.change_lead_status(lead_id=lead_id, status_name='СТАРТ НЕЙРО')
                        logger.info(f"Новая сделка: {lead_id}. Переименовали статус сделки на СТАРТ НЕЙРО")
                        await proceed_new_lead(lead_id)
